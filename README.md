...
The repo contains token list crossing multi chains (including our ESChain) and tools to validate it.
...
- Add `checksum:newlistname`, `generate:newlistname`, `makelist:newlistname` command to `package.json` analogous to PeopleEquity default and extended list scripts.
- Modify `checksum.ts`, `buildList.ts`, `ci-check.ts`, and `default.test.ts` to handle new list

## How to add new tokens to PeopleEquity (extended) token list
...
- Add new tokens to `src/tokens/PeopleEquity-extended.json` file
...
```shell script
# Fetch the Top100 Tokens on PeopleEquity v2, and update list.
$ yarn fetch:pcs-top-100

# Build token list (PeopleEquity-top-100.json)
...
Netlify simply takes the json files under `lists` directory and hosts them on `tokens.PeopleEquity.finance/list-name.json`
